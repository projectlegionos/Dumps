...
function handle_m_commonLog(){
    ...
    echo "Regular logs(such as android.log event.log) are kept in /common/ap_log !!!" > ${DATA_LOG_DEBUG_PATH}/NOTICE.txt
...
function transferDataVendor(){
    ...
    DIAG_LOGS_PATH=${DATA_VENDOR_LOG}/diag_logs
    TARGET_DIAG_LOGS=${newpath}/diag_logs

    if [ -d ${DATA_VENDOR_LOG} ]; then
        chmod 777 ${DATA_VENDOR_LOG} -R
        ALL_SUB_DIR=`ls ${DATA_VENDOR_LOG} | grep -v diag_logs`
        for SUB_DIR in ${ALL_SUB_DIR};do
    ...
    #special treatment for diag_logs
    if [ -d ${DIAG_LOGS_PATH} ]; then
        traceTransferState "mv /data_vendor/diag_logs"
        if [[ ! -d ${TARGET_DIAG_LOGS} ]];then
            mkdir -p ${TARGET_DIAG_LOGS}
        fi
        mv ${DIAG_LOGS_PATH}/* ${TARGET_DIAG_LOGS}
    fi
...
function transferBluetoothLog() {
    ...
    #Keliangwen@@CONNECTIVITY.BT.Basic.Log.5092143, coloect eardump log
    checkNumberSizeAndMove "/data/misc/eardump" "${newpath}/eardump"
}
...
function transfer_bluetooth() {
    ...
    # eardump
    #Keliangwen@@CONNECTIVITY.BT.Basic.Log.5092143, coloect eardump log
    MODULE_TARGET_PATH=${TARGET_PATH}/eardump
    checkNumberSizeAndMove "/data/misc/eardump" "${MODULE_TARGET_PATH}"
}
...
function dumpon(){
    ...
    if [ "${wlan_aging_test}" = "true" ] ; then
        ...
        adsp_crash_not_reboot_to_dump=true
